...
const REVIEWED_PATH = join(OUT_DIR, "doa-monsters.reviewed.json");
...
if (problems.length === 0) {
  ...
  // Review decisions live in their own ledger and never touch the base
  // package: the reviewed package is a second output, a pure function of the
  // specs plus the ledger, exactly as the Enraged Eggplant pipeline treats its
  // reviewed candidate.
  const ledger = readLedger(".", SRD_LEDGER_PATH);
  const { reviewed, decisions, problems: reviewProblems } = applyReviewDecisions(records, ledger);
  problems.push(...reviewProblems);
  const reviewedPkg = { manifest: manifest(VERSION, reviewed.length), monsters: reviewed };
  problems.push(...validatePackage(reviewedPkg, { allowUnapproved: true }));

  const files = [
    [PACKAGE_PATH, `${JSON.stringify(pkg, null, 2)}\n`],
    [REVIEWED_PATH, `${JSON.stringify(reviewedPkg, null, 2)}\n`],
    [CHECKLIST_PATH, checklist(reviewedPkg, decisions)],
  ...
  if (problems.length === 0) {
    const approved = reviewed.filter(record => record.provenance.manualReviewStatus === "approved").length;
    const status = `${approved}/${reviewed.length} approved`;
    console.log(check
      ? `SRD monster artifacts are current (${unchanged} file(s), ${records.length} record(s), ${status}).`
      : `Wrote ${written} and left ${unchanged} unchanged of ${files.length} file(s) for ${records.length} record(s), ${status}.`);
  }
}
...
function checklist(pkg, decisions) {
  const approved = pkg.monsters.filter(record => record.provenance.manualReviewStatus === "approved").length;
  const rows = pkg.monsters.map(record => {
    ...
    return `| ${record.name} | ${covers} | ${record.stats.attributes.st} | `
      ...
      + `${entry ? `${entry.reviewer}, ${entry.reviewedOn}` : "—"} |`;
  });
  return [
    "<!-- Generated by scripts/build-srd-monsters.mjs from content/srd-monsters/ and",
    `     ${SRD_LEDGER_PATH}. Do not edit; run npm run build:srd-monsters. -->`,
    "",
    ...
    `${pkg.monsters.length} independently authored record(s), version ${pkg.manifest.version}; `
      + `${approved} approved.`,
    "",
    `Statuses come from \`${SRD_LEDGER_PATH}\`. Approval needs the two gates the review workflow`,
